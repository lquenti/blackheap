...
import { benchmark_type_str, evaluate, is_read_op_str } from '../utils/ModelUtils';
...
const UnifiedFunctionPlot = () => {
    ...
    for (const m of model) {
        ...
        const line = {
            ...
            name: `${benchmark_type_str(m.benchmark_type)}: ${is_read_op_str(m.is_read_op)}`,
            marker: { color: `#${[1, 2, 3, 4, 5, 6].map(_ => "0123456789ABCDEF"[Math.floor(Math.random() * 16)]).join('')}` }, // TODO COLOUR
        };
        data.push(line);
    }
    return (

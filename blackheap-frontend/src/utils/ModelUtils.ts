...
const is_read_op_str = (is_read: boolean): string =>
  is_read ? "Read" : "Write";
...
const linear_equation = (linear: Linear): string =>
  `f(x) = ${linear.slope} x + ${linear.y_intercept}`;
const constant_linear_equation = ({
  constant,
  linear,
}: ConstantLinear): string => {
...
const evaluate_constant_linear = (f: ConstantLinear, x: number): number => {
  if (in_interval(f.linear.valid_interval, x))
    return evaluate_linear(f.linear, x);
...
const evaluate = (
  ...
  if ("ConstantLinear" in f)
    return evaluate_constant_linear(f.ConstantLinear, x);
  if ("Linear" in f) return evaluate_linear(f.Linear, x);
  throw new Error(`${f} couldn't get parsed`);
};

const get_max_for_access_sizes = (

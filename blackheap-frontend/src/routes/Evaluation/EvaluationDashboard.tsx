...
const EvaluationDashboard = ({
  ...
  preloadeeRecords: PreloadeeRecords;
}) => {
  ...
  const [classifiedData, setClassifiedData] =
    useState<ClassifiedPreloadeeRecords | null>(null);

  const [lowerSelection, setLowerSelection] = useState<number>(0);
  const [upperSelection, setUpperSelection] = useState<number>(
  ...
  const allClassificationTypes = [
    ...new Set(model.map((m) => m.benchmark_type)),
  ];
  ...
  const updateView = () => {
    const lower =
      parseInt((document.getElementById("lower") as HTMLInputElement).value) ||
      0;
    const upper =
      parseInt((document.getElementById("upper") as HTMLInputElement).value) ||
      Number.POSITIVE_INFINITY;
  ...
  return (
    ...
    <div className="mx-auto max-w-2xl">
      <h1 className="text-4xl underline mt-9 mb-3">Classification Report</h1>
      ...
      <div>
        <div className="flex items-center mx-auto shadow rounded border-0 py-3">
          <input
            type="number"
            className="flex-grow input bg-neutral input-bordered mr-3"
            id="lower"
            placeholder="Lower Bound"
          />
          <input
            type="number"
            className="flex-grow input bg-neutral input-bordered mx-3"
            id="upper"
            placeholder="Upper Bound"
          />
          <button className="btn btn-primary ml-3" onClick={updateView}>
            Update
          </button>
        </div>
      </div>
